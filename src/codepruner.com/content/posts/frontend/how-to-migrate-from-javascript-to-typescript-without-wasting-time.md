...
5. If you have to touch a `.js` file, to modify or to import from `.ts` file. Then
   ...
   * Everywhere where are no types use `any`
     * or I recommend to use `fakeAny`. You can read more about that in post [How to handle `noImplicitAny` during migration fom js to ts](how-to-handle-not-implicit-any-during-migration-fom-js-to-ts)
